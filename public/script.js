...
const currentTheme = "dark" // Force dark theme
  ...
  transactions.forEach((transaction) => {
    ...
    const transactionItem = `
            <div class="transaction-item ${isSender ? "sent" : "received"}" data-transaction-id="${transaction.id}">
                ...
                <button class="transaction-receipt-btn" onclick="showReceipt('${transaction.id}')">
...
function initializeUI() {
  ...
  // Theme selector - Force dark theme
  document.body.setAttribute("data-theme", "dark")
...
function createReceiptModal() {
  ...
  if (!document.getElementById("receipt-modal")) {
    ...
    receiptModal.innerHTML = `
            <div class="receipt-container">
                ...
                <div class="receipt-actions">
                    <button id="receipt-back-btn" class="receipt-back-btn">
                        <i class="fas fa-arrow-left"></i> နောက်သို့
                    </button>
                    <button id="receipt-download-btn" class="receipt-download-btn">
                        <i class="fas fa-download"></i> ဒေါင်းလုပ်ဆွဲရန်
                    </button>
                </div>
            </div>
        `
    ...
    // Add event listeners
    document.getElementById("receipt-back-btn").addEventListener("click", hideReceipt)
...
async function showReceipt(transactionId) {
  try {
    ...
    // Get user phone
    const { data: userData } = await supabase.from("users").select("phone").eq("user_id", currentUser.user_id).single()
    ...
    // Create receipt HTML
    const receiptHTML = `
            <div class="receipt" id="receipt-for-download">
                <div class="receipt-watermark">Opper Pay</div>
                <div class="receipt-header">
                    <div class="receipt-logo">
                        <div class="receipt-logo-icon">
                            <i class="fas fa-wallet"></i>
                        </div>
                        <div class="receipt-logo-text">Opper Pay</div>
                    </div>
                    <div class="receipt-title">ငွေလွှဲပြေစာ</div>
                    <div class="receipt-subtitle">Transaction ID: ${transaction.id}</div>
                </div>
                <div class="receipt-body">
                    ...
                    <div class="receipt-status success">
                        ${isSender ? "ငွေပို့" : "လက်ခံရရှိသည်"}
                    </div>
                    <div class="receipt-info-item">
                        <div class="receipt-info-label">မှ:</div>
                        <div class="receipt-info-value">${transaction.from_phone}</div>
                    </div>
                    <div class="receipt-info-item">
                        <div class="receipt-info-label">သို့:</div>
                        <div class="receipt-info-value">${transaction.to_phone}</div>
                    </div>
                    <div class="receipt-info-item">
                        <div class="receipt-info-label">ရက်စွဲ:</div>
                        <div class="receipt-info-value">${formattedDate}</div>
                    </div>
                    <div class="receipt-info-item">
                        <div class="receipt-info-label">အချိန်:</div>
                        <div class="receipt-info-value">${formattedTime}</div>
                    </div>
                    ${
                      transaction.note
                        ? `
                    <div class="receipt-info-item">
                        <div class="receipt-info-label">မှတ်ချက်:</div>
                        <div class="receipt-info-value">${transaction.note}</div>
                    </div>
                    `
                        : ""
                    }
                    <div class="receipt-info-item">
                        <div class="receipt-info-label">ငွေပေးချေမှုစနစ်:</div>
                        <div class="receipt-info-value">Opper Pay</div>
                    </div>
                </div>
                <div class="receipt-footer">
                    ဤပြေစာသည် Opper Pay မှ အလိုအလျောက်ထုတ်ပေးသည့် တရားဝင်ပြေစာဖြစ်ပါသည်။
                </div>
            </div>
        `
    ...
    console.error("Show receipt error:", error)
  }
}
...
async function downloadReceipt() {
  ...
  html2canvas(receiptElement, {
    ...
    .then((canvas) => {
      ...
      link.download = `opper-pay-receipt-${currentReceipt.id}.png`
...
function setupFormSubmissions() {
  ...
  // Check account existence when phone number is entered
  transferPhoneInput.addEventListener("blur", async () => {
    ...
    try {
      ...
      if (error || !recipient) {
        ...
        accountStatusElement.textContent = "အကောင့်မတွေ့ရှိပါ"
        accountStatusElement.classList.remove("exists")
        accountStatusElement.classList.add("not-exists")
        accountStatusElement.style.display = "block"
      } else {
        // Account exists
        accountStatusElement.textContent = `အကောင့်ရှိသည် (ID: ${recipient.user_id})`
        accountStatusElement.classList.remove("not-exists")
        accountStatusElement.classList.add("exists")
        accountStatusElement.style.display = "block"
      }
    } catch (error) {
